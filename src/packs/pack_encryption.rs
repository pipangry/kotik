...
where F: Fn(&str, OsString) -> Result<(), PackEncryptionError> {
    if args.is_empty() {
        return Err(String::from("No arguments provided. Use 'help' to get list of all available commands."));
    }
    
    // For cases then user somehow forgot to specify path
    let path_arg = if args[1..].is_empty() {
        let mut input = String::new();
        get_input("It looks like you forgot that you need to specify the folder path. Enter path:", &mut input);
        input
    } else {
